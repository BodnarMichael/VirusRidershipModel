...
[
  ...
  tolerance           ;; Maximum reproduction rate a person is willing to take to use public transport. MBE
  riding?             ;; If true, the person uses public transport. MBE

  local-infection-rate ;; how many turtles in the environment are infected
]
...
to setup-people
  ...
  [
    ...
    set tolerance random-normal global-tolerance ( global-tolerance / 4 )
    ;;set tolerance global-tolerance ;; MBE (later we will put a slider in the interface)
    set riding? true
  ]
end
...
to update-riding   ;; turtle procedure
  let local-turtles count turtles in-radius env-size
  let local-infected count turtles in-radius env-size with [ infected? ]
  ;;let local-patch neighbors
  ;;let local-infected count ( ( turtles with [  infected? ])  of local-patch )
  ;;let local-turtles count turtles of local-turtles
  ifelse local-turtles > 0
    [ set local-infection-rate ( local-infected / local-turtles ) ]
    [ set local-infection-rate 0 ]
  ifelse ( local-infection-rate >= tolerance )
    [ set riding? false ]
    [ set riding? true ]
   ...
   let nearby-uninfected (turtles-on neighbors)
     ...
     [ ask nearby-uninfected
       [ ifelse riding?
          ...
          [ if random-float 100 < (infection-chance * risk-reduction-biking)
            [ set infected? true
              set nb-infected (nb-infected + 1)
            ]
          ]     ;; MBE
...
"" ""
PENS
"riding" 1.0 0 -16777216 true "" "plot (((count turtles with [ riding? ]) / initial-people) )"
"infected" 1.0 0 -5298144 true "" "plot (((count turtles with [ infected? ]) / initial-people) )"
"tol" 1.0 0 -955883 true "" "plot global-tolerance"
"r0" 1.0 0 -13345367 true "" "plot r0"
"100%" 1.0 0 -5987164 true "" "plot 1"
...
"" ""
PENS
"mean" 1.0 0 -13345367 true "" "plot mean [local-infection-rate] of turtles"
"var" 1.0 0 -7500403 true "" "plot variance [local-infection-rate] of turtles"
"pen-2" 1.0 0 -2674135 true "" "plot ( global-tolerance - ( global-tolerance / 4 ) )"
"pen-3" 1.0 0 -2674135 true "" "plot ( global-tolerance + ( global-tolerance / 4 ) )"
...
"" ""
PENS
"Tolerance" 0.01 1 -16777216 true "" "histogram [tolerance] of turtles"
